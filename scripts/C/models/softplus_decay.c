...
#include <math.h>
#include "softplus_decay.h"

/*------------------------  MODEL PARAMETERS  -------------------------*/

const struct Params default_params = {0.5, 0.1, 0.01, 20., .001, 0.2, 2.*M_PI/360., .9, 0.0001, 0.02};
...
double foi(double H, double I, Params *params){
	double Fmin = (1.-params->Mmax)*exp(-H/params->delta);
	double slope = params->Mmax/params->Hthres;
	return (slope*params->epsilon*log(exp(1/params->epsilon*(params->Hthres-H))+1)+Fmin)*I*params->beta;
}
